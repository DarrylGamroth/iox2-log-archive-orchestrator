...
| `ORCH-008` | Failure modes MUST return stable exit codes and explicit error payloads. | `src/error.rs` (`exit_code`, structured payload), `src/command.rs` error mapping | `missing_action_returns_deterministic_invalid_input_error` | Covered |
...
| `DF-009` | Configuration precedence MUST be `CLI > env > config file > profile defaults`. | `src/config.rs` (`resolve`) | `config::tests::cli_overrides_defaults` + merge logic inspection | Covered |
